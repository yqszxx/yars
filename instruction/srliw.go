...
// SRLIW is an RV64I-only instruction that is analogously defined but operates on 32-bit values and
// produces signed 32-bit results. It generates an illegal instruction exception if imm[5] != 0
var SRLIW = Instruction{
	name:    "SRLIW",
	pattern: bv.P("0000000 XXXXX XXXXX 101 XXXXX 0011011"), // it won't match if imm[5] != 0 instead of generating an exception
	operation: func(_inst *Instruction) {
		log.Printf("Decoding as SRLIW x%d, x%d, %d",
			_inst.rd.ToUint64(),
			_inst.rs1.ToUint64(),
