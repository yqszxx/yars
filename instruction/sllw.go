...
// SLLW is an RV64I-only instruction that is analogously defined but operates on 32-bit values
// and produces signed 32-bit result. The shift amount is given by rs2[4:0].
var SLLW = Instruction{
	name:    "SLLW",
	pattern: bv.P("0000000 XXXXX XXXXX 001 XXXXX 0111011"),
	operation: func(_inst *Instruction) {
		log.Printf("Decoding as SLLW x%d, x%d, x%d",
			_inst.rd.ToUint64(),
			_inst.rs1.ToUint64(),
			_inst.rs2.ToUint64())
