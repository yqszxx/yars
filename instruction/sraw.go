...
// SRAW is an RV64I-only instruction that is analogously defined but operates on 32-bit values
// and produces signed 32-bit result. The shift amount is given by rs2[4:0].
var SRAW = Instruction{
	name:    "SRAW",
	pattern: bv.P("0100000 XXXXX XXXXX 101 XXXXX 0111011"),
	operation: func(_inst *Instruction) {
		log.Printf("Decoding as SRAW x%d, x%d, x%d",
			_inst.rd.ToUint64(),
			_inst.rs1.ToUint64(),
			_inst.rs2.ToUint64())
		...
		result.From(uint32(int32(op1.ToUint64()) >> uint32(op2.ToUint64()))) // perform computation
